...
af = File_config.getfloat('ChucNang', 'delay_click')

# File_config.get('ChucNang', 'delay_click')
...
class controller:
    ...
    Delay_Click = File_config.getfloat('ChucNang', 'delay_click')
    ...
    def Click(self, x, y):
        ...
        time.sleep(float(self.Delay_Click))
...
class Bot_ChucNang:
    ...
    Delay_Load_Image = File_config.getint('ChucNang', 'delay_load_image')

   # Click_Black = None
    if File_config.get('ChucNang', 'black') == "Click":
        Click_Black = True
...
# Func Controller  
    ...
    def Time_Check(self,Start_Time,Stop_Time = 0):
        ...
        if Stop_Time == 0:
            ...
            if time.time() - int(Start_Time) >= int(self.Delay_Load_Image):
                return True 
            else:
                return False
        ...
        if maxVal >= threshold:
            ...
            if x + khoang_x >= int(anhnho.toado()[0]) >= x - khoang_x and y + khoang_y >= int(
                ...
                if self.Running:
                    ...
                    if Exit_Event.wait(timeout=0.05): return False
        ...
        for i in range(0,1000):
            ...
            if  self.Running:
                ...
                if self.Check(image_SkillLanhChua.MoSach,) and self.Check(image_DangNhap.ThanhBenTrong) and self.Check(image_TongQuan.Mo_TongQuan) or self.Check(image_TongQuan.Mo_TongQuan) and self.Check(image_DangNhap.ThanhBenNgoai) and self.Check(image_SkillLanhChua.MoSach,) :
                    ...
                    if  self.Running:
                        if self.Check(image_close_QuangCao.Keyblack ) or  self.Check(image_close_QuangCao.Keyblack2):
                            if self.Click_Black:
                                print("Send Click")
                                controller().Click(14, 43)
                        ...
                        if self.Check(image_TongQuan.Dong_TongQuan, Click=True,sleep =0.08):
                            ...
                            if self.Time_Check(Start,float(self.Delay_Load_Image)/2):
                                controller().KeyEvent("4")
                                Start = time.time()
                ...
                while not self.Running: Exit_Event.wait(timeout= 0.05) ; Start = time.time()
...
# Chuc Nang
    ...
    def Kick_Hoat_Vip(self):
        ...
        for i in range(self.TimeLoad[0],self.TimeLoad[1]):
            if Exit_Event.wait(timeout=0.05): return
                ...
                if self.Running:
                    ...
                    if self.Check(image_DiAnMo.Find_Di):
                        ...
                        if cap_hientai == capmo or Ck_Di == True:
                            if self.Check(image_DiAnMo.Find_Di, Click=True,sleep =2):
                                return True
                            else:
                                if self.Time_Check(Start,2):
                                    return
...
# Event
            ...
            for i in range (0,1000):
                ...
                if self.Running:
                    ...
                    if Tap_Hop:
                        ...
                        if self.Check(image_Event.TapHop,Click=True,sleep=2):
                            return
                    ...
                    if CK_DiQuanXamLuoc:
                        if self.Check(image_Event.CK_DiQuanXamLuoc,sleep=0.2):
                            if self.Check(image_Event.NguoiKhongLo, khoang_y = 10):
                                if self.Check(image_Event.DiTieuDiet , Click=True,sleep = 2):
                                    CK_DiQuan = True
                                    CK_DiQuanXamLuoc = False
                                    Start = time.time()
                                    continue
    ...
    def VienTro_TaiNguyen(self,LoaiTaiNguyen : int):

        VaoToaDo = False
        RaNgoaiThanh = True
        Thuongmai = False
        CK_DanhDau =False
        TiepTeNguonLuc = False
        Vao_TiepTeNguonLuc = False
        ...
        if self.out_MHchinh():
            ...
            for i in range(0,9000):
                ...
                if Thuongmai:
                    if self.Check(image_VienTro.Thuongmai,Click=True):
                        return

                if TiepTeNguonLuc:
                    if self.Check(image_VienTro.CK_TiepTeNguonLuc):
                        if LoaiTaiNguyen == 1:
                            controller().Swipe("307 613 686 613")
                            Thuongmai = True
                            Start = time.time()
                            continue
                        
                        if LoaiTaiNguyen == 2:
                            controller().Swipe("307 883 686 883")
                            Thuongmai = True
                            Start = time.time()
                            continue
                        if LoaiTaiNguyen ==3:
                            controller().Swipe("307 1085 686 1085")
                            Thuongmai = True
                            Start = time.time()
                            continue

                        if LoaiTaiNguyen == 4:
                            controller().Swipe("307 1322 686 1322")
                            Thuongmai = True
                            Start = time.time()
                            continue
                ...
                if Vao_TiepTeNguonLuc:
                    if self.Check(image_VienTro.TiepTeNguonLuc,Click=True):
                        TiepTeNguonLuc = True
                        Vao_TiepTeNguonLuc = False
                        Start = time.time()
                        continue
                ...
                if CK_DanhDau:
                    if self.Check(image_VienTro.Sao1,Click=True):
                        CK_DanhDau = False
                        Vao_TiepTeNguonLuc = True
                        Start = time.time()
                        continue
                ...
                if VaoToaDo:
                    if self.Check(image_VienTro.VaoToaDo,Click=True):
                        VaoToaDo = False
                        CK_DanhDau = True
                        Start = time.time()
                ...
                if RaNgoaiThanh:
                    if self.Check(image_DangNhap.ThanhBenTrong, Click=True) or self.Check(image_DangNhap.ThanhBenNgoai):
                        VaoToaDo =True
                        RaNgoaiThanh = False
                        Start = time.time()
...
# Quais KVK
    ...
    def DoKhoangCach(self):
        ...
        for i in range(self.TimeLoad[0],self.TimeLoad[1]):
            if Exit_Event.wait(timeout=0.05): return Khoangcach
